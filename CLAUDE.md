...
Desktop (Tauri): the app has no server code, so `src-tauri/tauri.conf.json` builds it with `adapter-static` instead — `beforeBuildCommand` runs `npm run build:tauri`, which sets `TAURI_BUILD=true` so `svelte.config.js` swaps the adapter (plain `npm run build` / Docker still use `adapter-node`, untouched). `npm run tauri build` produces both a `.exe`/`.msi` installer (in `src-tauri/target/release/bundle/`) and the raw `qrding.exe` binary it wraps, which already runs standalone (the WebView2 loader is linked in, and Windows 11 ships the WebView2 runtime). Building needs a Rust toolchain and, on Windows, the MSVC Build Tools. `.github/workflows/release-desktop.yml` only ships the raw exe — `tauri build --no-bundle` skips NSIS/MSI entirely — renamed to `QRding-x64-portable.exe`. It runs on every push to `main` (plus manual `workflow_dispatch`) and republishes it as the rolling `latest` GitHub Release (a pre-release, deleted and recreated each run via `gh release delete`/`gh release create` — not a versioned release), so `main` always has a current desktop build available. A `concurrency` group cancels an in-flight run when a newer push arrives, so an older build can't overwrite a newer one. It's a separate workflow from `ci.yml`, not a job inside it, since a from-scratch Rust compile on a Windows runner is slow and shouldn't block PR feedback.
...
- **Download/copy:** files are only encoded on click. `downloadBlob` revokes its object URL after 10 s, because revoking immediately can cancel downloads. A status line under the buttons (`showActionStatus`) confirms each export or copy; in the desktop app, WebView2 saves downloads to the Downloads folder without any UI, so that line is the only feedback there.
